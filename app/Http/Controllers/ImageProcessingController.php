...
class ImageProcessingController extends Controller
{
    public function processImage(Request $request): JsonResponse
    {
        try {
            ...
            $imageDescription = $this->convertImageToText($fullImagePath);
            ...
            $audioUrl = $this->convertTextToSpeech($imageDescription);
    ...
    private function convertImageToText(string $imagePath): ?string
    {
        try {
    ...
    private function convertTextToSpeech(string $text): ?string
    {
        try {
            ...
            $elevenLabsVoiceId = env('ELEVEN_LABS_VOICE_ID', 'pNInz6obpgDQGcFmaJgB'); // Adam voice
            ...
            ])->timeout(30)->post("https://api.elevenlabs.io/v1/text-to-speech/{$elevenLabsVoiceId}", [
                'text' => $text,
