        ...
        <div class="mt-6">
          ...
          <div class="mt-6">
            <button
              ...
              :disabled="authLoading"
              type="button"
              class="w-full flex justify-center items-center px-4 py-2 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:bg-gray-100 disabled:cursor-not-allowed transition-colors"
            >
...
const loginWithGooglePopup = async () => {
  ...
  try {
    ...
    authMessage.value = 'Google authentication failed. Please try again.';
    authMessageType.value = 'error';
    authLoading.value = false;
  }
};
  ...
  if (code) {
    ...
    try {
      ...
      if (data.success) {
        ...
        authMessage.value = data.message || 'Google authentication failed';
        authMessageType.value = 'error';
      }
    } catch (error) {
      console.error('Google callback error:', error);
      authMessage.value = 'Authentication failed. Please try again.';
      authMessageType.value = 'error';
    } finally {
      authLoading.value = false;
    }
...
onMounted(async () => {
  ...
  await handleGoogleCallback(); // Add this line
